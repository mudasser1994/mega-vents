...
exports.eventUpdated = functions.firestore.document("events/{eventId}")
  .onUpdate(async (snapshot, context) => {
    ...
    if (before.attendees.length < after.attendees.length) {
      ...
      try {
        ...
        return console.log("error is here ",error)
      }
    }
      ...
      try {
        const followersDoc = await db.collection("following").doc(attendeeLeft.id).collection("userFollowers").get();
        followersDoc.forEach(doc => {
          const post = newPost(attendeeLeft, "left-event", context.params.eventId , before);
          admin.database().ref(`/posts/${doc.id}`).push(post);
        })
      }
      catch (error) {
